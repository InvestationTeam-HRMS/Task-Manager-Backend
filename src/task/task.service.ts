...
export class TaskService {
  ...
  async create(
    ...
    if (files && files.length > 0) {
      ...
      for (const file of files) {
        if (!file.size || file.size === 0 || (!file.buffer && !file.path))
          continue;
  ...
  async submitForReview(
    ...
    try {
      this.logger.log(
        `submitForReview called - id: ${id}, userId: ${userId}, remark: ${remark}, filesCount: ${files?.length || 0}`,
      );
      ...
      const isAlreadyInReview =
        (task.taskStatus as any) === TaskStatus.ReviewPending;
      ...
      if (files && files.length > 0) {
        this.logger.log(
          `Processing ${files.length} files for task ${task.taskNo}`,
        );
        const savedUrls: string[] = [];
        for (const file of files) {
          try {
      ...
      this.logger.error(
        `submitForReview failed: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async bulkUpload(file: Express.Multer.File, userId: string) {
    ...
    const tempExcelPath =
      file.path || path.join(uploadDir, `bulk_${Date.now()}.xlsx`);
    const tempCsvPath = path.join(uploadDir, `bulk_${Date.now()}.csv`);
