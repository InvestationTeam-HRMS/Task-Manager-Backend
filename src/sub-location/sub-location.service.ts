...
export class SubLocationService {
  ...
  async findAll(pagination: PaginationDto, filter?: FilterSubLocationDto) {
    ...
    if (sortBy === 'groupNo' || sortBy === 'groupName') {
      ...
      // Check if field exists on SubLocation model, otherwise fallback to createdAt
      const validFields = ['id', 'subLocationNo', 'subLocationName', 'subLocationCode', 'clientGroupId', 'companyId', 'locationId', 'address', 'status', 'remark', 'createdAt', 'updatedAt'];
  ...
  async downloadExcel(query: any, userId: string, res: any) {
    ...
    const columns = [
      ...
      { header: 'SubLocation No.', key: 'subLocationNo', width: 15 },
      { header: 'Sublocation', key: 'subLocationName', width: 30 },
      { header: 'Sublocation Code', key: 'subLocationCode', width: 15 },
