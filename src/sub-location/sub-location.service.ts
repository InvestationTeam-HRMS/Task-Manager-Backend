...
export class SubLocationService {
  ...
  async handleBackgroundUpload(payload: {
    ...
    try {
      const result = await this.bulkCreate({ subLocations: data }, userId);

      await this.notificationService.createNotification(userId, {
        title: 'Sub-Location Import Completed',
        description: `Successfully imported ${result.success} sub-locations from ${fileName}. Failed: ${result.failed}`,
        type: 'SYSTEM',
