...
export class PdfService {
    ...
    async generateReport(title: string, sections: ApiSection[], filename: string = 'api-documentation.pdf'): Promise<string> {
    ...
    private addSectionHeader(doc: PDFKit.PDFDocument, text: string) {
    ...
    private addText(doc: PDFKit.PDFDocument, text: string) {
        doc.fontSize(10).font('Helvetica').text(text);
        doc.moveDown(0.3);
    }

    private addCode(doc: PDFKit.PDFDocument, code: string) {
        doc.fontSize(9)
            .font('Courier')
            .fillColor('#1e293b')
            .text(code, { indent: 20 });
    ...
    private addApiEndpoint(doc: PDFKit.PDFDocument, api: ApiEndpoint) {
        // API Name
        ...
        doc.fontSize(10).font('Helvetica-Bold').fillColor(methodColor).text(api.method, { continued: true });
        ...
        doc.font('Helvetica').text(api.authRequired ? 'Required (Bearer Token)' : 'Not Required');
